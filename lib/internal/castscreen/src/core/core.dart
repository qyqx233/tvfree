...
abstract final class CastScreen {
  ...
  static Future<List<Device>> discoverByIP({
    ...
    _clients.clear();
    _devices.clear();
    // await _init(ipv4, ipv6, port, true, onError);
    await _createLocalSocket(port, true, onError);
    _searchIP(ST, ip, port);
  ...
  static Future<void> _createLocalSocket(
    int port,
    bool fetchDevice,
    Function(Exception) onError,
  ) async {
  ...
  static void _searchIP(String ST, String ip, int port) {
    ...
    for (var socket in _sockets) {
      socket.send(data, InternetAddress(ip), 1900);
    }
  }
